...
def peak_finder(x0, thresh=None, extrema=1):
    ...
    ----------
    x0 : 1d array
        A real vector from which the extrema will be found.
    ...
    https://github.com/mne-tools/mne-python/blob/master/mne/preprocessing/peak_finder.py

    Example
    -------
    t = np.arange(0,10,0.0001)
    x = 12*np.sin(10*2*np.pi*t)-3*np.sin(.1*2*np.pi*t)+np.random.randn(len(t))
    x[1250:1255] = np.max(x);
    peak_loc,peak_mag = peak_finder(x)
    """
    ...
    if thresh is None:
        thresh = np.abs((np.max(x0) - np.min(x0)) / 4.)
    ...
    # Find where the derivative changes sign (add one to recover index
    # in original data):
    ind = np.where(dx0[:-1] * dx0[1:] < 0)[0] + 1

    # Include endpoints in potential peaks and valleys:
    x = np.r_[x0[0], x0[ind], x0[-1]]
